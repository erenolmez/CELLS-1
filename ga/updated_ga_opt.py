"""
ga_variable_antennas.py  –  GA with variable antenna counts per cell
====================================================================
Search space:  (0‒2 antennas)^{36}  ≈ 10^17 layouts.
Fitness = failures + λ_r * redirects + λ_a * antenna_count.
Prints detailed stats each generation.
"""
...
EP_STEPS           = 240         # 10 simulated days
SEED               = 42
...
def eval_layout(ch: Chromosome) -> Tuple[float,int,int,int]:
    ...
    for _ in range(EP_STEPS):
        env.step(None)                # fixed layout
    _, fails, reds = env.check_coverage()
    ant_cnt = np.sum(env.antenna_grid)
    fitness = fails + LAMBDA_REDIRECT*reds + LAMBDA_ANTENNA*ant_cnt
...
for gen in range(GENERATIONS):
    ...
    print(f"Gen {gen:03d} | bestF={evaluated[gen_best_idx][1]:4d} "
          f"bestR={evaluated[gen_best_idx][2]:4d} cost={evaluated[gen_best_idx][3]:2d} "
          f"| meanF={mean_fail:6.1f} meanR={mean_redir:6.1f} costµ={mean_cost:4.1f}")
